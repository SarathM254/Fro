...
export default function StaffManagementPortal({ onBack }) {
  ...
  const [editingStaff, setEditingStaff] = useState(null); // If null, the popup treats the template as "New Account"

  // --- LOCAL FORM FIELDS BUFFER STATE ---
  const [formData, setFormData] = useState({ name: '', code: '', role: 'Salesman' });

  // Mock Staff Master Roster Dataset Array
  const [staffRoster, setStaffRoster] = useState([
    { _id: "st_101", name: "K. Ramesh Rao", code: "M-SR-01", role: "Salesman" },
    { _id: "st_102", name: "Anitha Reddy", code: "M-OP-04", role: "Operator" },
    { _id: "st_103", name: "S. K. Mahendra", code: "M-SR-02", role: "Salesman" }
  ]);

  const openCreateModal = () => {
    setEditingStaff(null);
    setFormData({ name: '', code: '', role: 'Salesman' });
    setIsFormModalOpen(true);
  };

  const openEditModal = (staffMember) => {
    setEditingStaff(staffMember);
    setFormData({ name: staffMember.name, code: staffMember.code, role: staffMember.role });
    setIsFormModalOpen(true);
  };

  // --- CRITICAL FEATURE IMPLEMENTATION: SAFE-DELETE SECURITY GATE ---
  const handleSafeDeleteRequest = (staffMember) => {
    const messageTemplate = `Are you absolutely sure you want to delete account: ${staffMember.name} (${staffMember.code})?\n\nCRITICAL AUDIT NOTICE:\nEnsure all outstanding market Brought Forward (BF) debt balances have been completely cleared and logged in the master ledger before removal!`;
    
    if (window.confirm(messageTemplate)) {
      setStaffRoster(prev => prev.filter(item => item._id !== staffMember._id));
      console.log(`[DB Write] Terminated account authorization for reference node key: ${staffMember._id}`);
    }
  };

  const handleCommitFormSubmission = (e) => {
    e.preventDefault();
    if (!formData.name.trim() || !formData.code.trim()) return;

    if (editingStaff) {
      // OVERRIDE MUTATION PATTERN: Update existing profile attributes inside the array mapping loop
      setStaffRoster(prev => prev.map(item => 
        item._id === editingStaff._id ? { ...item, ...formData } : item
      ));
      console.log(`[DB Update] Overwrote profile records details on node: ${editingStaff._id}`);
    } else {
      // INGESTION NEW PROFILE PATTERN: Push a fresh entry packet into the configuration array 
      const freshStaffPacket = {
        _id: `st_${Date.now()}`,
        ...formData
      };
      setStaffRoster(prev => [...prev, freshStaffPacket]);
      console.log("[DB Write] Inserted brand-new staff profile data asset node token.");
    }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-xs transition-opacity duration-200">
          <div className="w-full max-w-sm bg-white rounded-2xl border border-slate-200 shadow-xl overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            ...
            <form onSubmit={handleCommitFormSubmission} className="p-5 space-y-4">
              ...
              {/* Input Item 1: Full Representative Name */}
              ...
              {/* Input Item 3: Operational Functional System Role Dropdown */}
              <div className="space-y-1.5">
              ...
              {/* Action Buttons Drawer Stack Footer */}
