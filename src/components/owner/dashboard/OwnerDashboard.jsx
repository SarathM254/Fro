...
export default function OwnerDashboard({ onNavigate }) {
  ...
  const [systemOperationalDate, setSystemOperationalDate] = useState(new Date("2026-06-11"));
  
  const mockFinancials = {
    totalSalesmanBFDebt: 142850,
    stockDelivered: { billed: 180.00, unbilled: 29.50 }
  };
  ...
  */
  ...
  const totalStockVolumeM = useMemo(() => {
    return mockFinancials.stockDelivered.billed + mockFinancials.stockDelivered.unbilled;
  }, [mockFinancials]);
  ...
  const handleSystemDatePushForward = () => {
    ...
    if (dayOfWeek === 6) {
      // It's Saturday: Perform Context Leap over Sunday straight to Monday (+2 Days)
      nextDate.setDate(nextDate.getDate() + 2);
    } else {
      // Standard Weekday Shift: Push forward normally (+1 Day)
  ...
  const portalWindows = [
    { id: "verify", title: "Verification Desk", desc: "Approve operator-slashed bills & cashout sheets", countBadge: "3 Pending", badgeColor: "bg-rose-50 text-rose-700 border-rose-100", svg: <svg className="w-5 h-5 text-indigo-600" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> },
        ...
        <div className="space-y-3">
          ...
          <div className="bg-white rounded-2xl border border-slate-200/80 p-4 shadow-2xs flex justify-between items-center">
            ...
            <span className="text-xl font-black text-rose-700 tracking-tight bg-rose-50/50 border border-rose-100/70 px-3 py-1.5 rounded-xl">
              ₹{mockFinancials.totalSalesmanBFDebt.toLocaleString('en-IN')}
            </span>
          </div>
            ...
            <div className="grid grid-cols-2 divide-x divide-slate-100 text-center">
              <div className="space-y-0.5 pr-2 text-left">
                ...
                <p className="text-base font-extrabold text-slate-800 tracking-tight">
                  {mockFinancials.stockDelivered.billed.toFixed(2)} <span className="text-xs font-medium text-slate-400">M</span>
                </p>
              </div>
              <div className="space-y-0.5 pl-4 text-left">
                ...
                <p className="text-base font-extrabold text-slate-800 tracking-tight">
                  {mockFinancials.stockDelivered.unbilled.toFixed(2)} <span className="text-xs font-medium text-slate-400">M</span>
                </p>
              </div>
            </div>
          </div>
                ...
                <div className="flex items-start gap-x-3.5">
                  ...
                  <div className="space-y-0.5">
                    <div className="flex items-center gap-x-2">
                      ...
                      <span className={`text-[9px] font-extrabold px-1.5 py-0.5 rounded-sm border ${portal.badgeColor}`}>{portal.countBadge}</span>
                    </div>
