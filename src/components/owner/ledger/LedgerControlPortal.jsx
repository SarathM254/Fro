...
export default function LedgerControlPortal({ onBack }) {
  ...
  // Mock Salesmen master records array
  const [salesmenList, setSalesmenList] = useState([
    { _id: "sm_01", name: "Ram", currentBF: 3070 },
    { _id: "sm_02", name: "K. Ramesh Rao", currentBF: 42100 },
    { _id: "sm_03", name: "S. K. Mahendra", currentBF: 0 }
  ]);
  ...
  */
  ...
  const handleCommitLedgerAdjustment = (e) => {
    e.preventDefault();
    const targetAmount = parseInt(inputAmount, 10);
    if (isNaN(targetAmount) || targetAmount <= 0) return;

    setSalesmenList(prevList => 
      prevList.map(salesman => {
        if (salesman._id !== modalContext.salesman._id) return salesman;

        // Perform balance adjustment arithmetic based on the active structural context button click
        const changeFactor = modalContext.type === 'increase' ? targetAmount : -targetAmount;
        const finalizedBF = Math.max(0, salesman.currentBF + changeFactor); // Prevents debt dropping below absolute 0

        return { ...salesman, currentBF: finalizedBF };
      })
    );

    console.log(`[DB Ledger Mutate] Action: ${modalContext.type.toUpperCase()}, Value: ₹${targetAmount}, Audit Note: "${adjustmentNote}"`);
            ...
            <div className="flex justify-between items-start">
              ...
              <div className="text-right space-y-0.5">
                ...
                <span className="text-base font-black text-slate-900 tracking-tight">
                  ₹{salesman.currentBF.toLocaleString('en-IN')}
                </span>
              </div>
            </div>
